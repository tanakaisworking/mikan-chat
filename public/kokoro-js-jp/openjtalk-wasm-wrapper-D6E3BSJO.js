// src/wasm/openjtalk-wasm.js
async function createOpenJTalkModule(moduleArg = {}) {
  var moduleRtn;
  ...
  function findWasmBinary() {
    if (Module["locateFile"]) {
      return locateFile("openjtalk-wasm.wasm");
    }
    return new URL("openjtalk-wasm.wasm", import.meta.url).href;
  }
  ...
  Module["FS"] = FS;
  var _ojt_configure, _ojt_g2p, _ojt_extract_full_context, _ojt_run_frontend, _ojt_synthesize, _free, _malloc, _emscripten_builtin_memalign, __emscripten_stack_restore, __emscripten_stack_alloc, _emscripten_stack_get_current, memory, __indirect_function_table, wasmMemory;
  function assignWasmExports(wasmExports2) {
    _ojt_configure = Module["_ojt_configure"] = wasmExports2["t"];
    _ojt_g2p = Module["_ojt_g2p"] = wasmExports2["u"];
    _ojt_extract_full_context = Module["_ojt_extract_full_context"] = wasmExports2["v"];
    _ojt_run_frontend = Module["_ojt_run_frontend"] = wasmExports2["w"];
    _ojt_synthesize = Module["_ojt_synthesize"] = wasmExports2["x"];
    _free = Module["_free"] = wasmExports2["y"];
    _malloc = Module["_malloc"] = wasmExports2["z"];
    _emscripten_builtin_memalign = wasmExports2["A"];
  ...
  var wasmImports = { b: ___cxa_throw, d: ___syscall_fcntl64, h: ___syscall_fstat64, j: ___syscall_ioctl, g: ___syscall_newfstatat, e: ___syscall_openat, n: __abort_js, o: __mmap_js, p: __munmap_js, m: _emscripten_resize_heap, k: _environ_get, l: _environ_sizes_get, f: _exit, a: _fd_close, i: _fd_read, q: _fd_seek, c: _fd_write };
  function run() {
    if (runDependencies > 0) {
...
var openjtalk_wasm_default = createOpenJTalkModule;

// src/wasm/openjtalk-wasm-wrapper.js
var DIC_FILES = [
  "sys.dic",
  "matrix.bin",
  "char.bin",
...
async function createOpenJTalkModule2(moduleArg = {}) {
  ...
  const ojtG2p = core.cwrap("ojt_g2p", "number", ["string", "number", "number", "number"]);
  const ojtExtract = core.cwrap("ojt_extract_full_context", "number", ["string", "number", "number"]);
  ...
  return {
    async configure(dicUrl, voiceUrl, dicArchiveUrl) {
      ...
      const rc = ojtConfigure(dicRoot, voicePath);
      ensureOk(rc, "configure");
    },
    g2p(text, kana) {
      const cap = Math.max(2048, text.length * 32);
      const outPtr = core._malloc(cap);
      try {
        const rc = ojtG2p(text, kana ? 1 : 0, outPtr, cap);
        ensureOk(rc, "g2p");
        return core.UTF8ToString(outPtr);
      } finally {
        core._free(outPtr);
      }
    },
    extractFullContext(text) {
      const cap = Math.max(16384, text.length * 256);
      const outPtr = core._malloc(cap);
      try {
        const rc = ojtExtract(text, outPtr, cap);
        ensureOk(rc, "extract_full_context");
        return decodeLineArray(core.UTF8ToString(outPtr));
      } finally {
        core._free(outPtr);
      }
    },
    synthesize(text, optionsJson) {
      const maxSamples = 48e3 * 20;
      const pcmPtr = core._malloc(maxSamples * 4);
      const ratePtr = core._malloc(4);
      try {
        const rc = ojtSynthesize(text, optionsJson, pcmPtr, maxSamples, ratePtr);
        ensureOk(rc, "synthesize");
        const sampleRate = core.HEAP32[ratePtr >> 2];
        const samples = core.HEAPF32.slice(pcmPtr >> 2, (pcmPtr >> 2) + rc);
        return { pcm: Array.from(samples), sampleRate };
      } finally {
        core._free(pcmPtr);
        core._free(ratePtr);
      }
    ...
    runFrontend(text) {
      const cap = Math.max(32768, text.length * 512);
      const outPtr = core._malloc(cap);
      try {
        const rc = ojtRunFrontend(text, outPtr, cap);
        ensureOk(rc, "run_frontend");
        return core.UTF8ToString(outPtr).split("\n").filter(Boolean).map((l) => JSON.parse(l));
      } finally {
        core._free(outPtr);
      }
    }
...
var openjtalk_wasm_wrapper_default = createOpenJTalkModule2;
export {
  createOpenJTalkModule2 as createOpenJTalkModule,
  openjtalk_wasm_wrapper_default as default
};
