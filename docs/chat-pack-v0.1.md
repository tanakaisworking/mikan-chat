...
- 画像、参照音声、待機モーションを同梱する場合は`assets/`以下へ置くMUST
- アセットのファイル名はASCII小文字、数字、ハイフン、ピリオドだけを使うMUST
- ZIP内のパス区切りには`/`を使うMUST
- `pack.json`から外部URL上の画像・音声やローカルファイルを直接アセットとして参照してはならないMUST NOT。認識済みのストリーミング拡張は例外とし、プレイヤーが接続先を明示するMUST
...
会話中のBGMは、パックへ同梱した音源ファイルを置くと自動で単曲ループします。自作または利用許諾のある音源（例：かまタマゴ素材）だけを使い、作者名・ライセンスをdiscovery.creditsへ記載するSHOULD。

```json
{
  "extensions": {
    "mikan.bgm": {
      "audio": "assets/bgm.m4a",
      "loop": true
    }
  }
}
...
mikan chatはassets/以下のwav、mp3、flac、m4aをBGMとして受け付け、会話画面で単曲ループします。ユーザーは同梱BGM・任意のAudio.com音源・音量を端末内へ保存して上書きできます。シナリオ指定もユーザー指定も無い場合はアプリ同梱のデフォルトBGMを再生します。ブラウザの自動再生制限が働く場合は、ユーザーがプレイヤーの再生ボタンを押します。

第三者の音源をパックへ同梱・指定してはならないMUST NOT。作者は自作または許諾済みの音源だけを使い、配布元の利用条件を確認するMUST。ユーザー自身がURLを指定する場合の許諾確認はユーザーの責任とします。

Audio.comのURL指定は公開サイト機能を利用した暫定実装です。商用公開の前に公式OAuth v1 APIと自社保有の音源へ移行するSHOULD。

### 16.3 mikan chatの待機モーション

人物の静止画に加えて短い待機モーションを同梱する場合は、人物の拡張へ無音MP4を指定します。`image`は読み込み中、モーション低減設定、再生失敗時のフォールバックとして残します。
...
```json
{
  ...
  "extensions": {
    "mikan.motion": {
      "idleVideo": "assets/character-idle.mp4"
    }
  }
}
```

mikan chatは動画を無音、自動再生、単曲ループ、インライン再生します。参照実装は`.mp4`のみを受け付け、1本8 MiBまでです。作者はH.264、音声トラックなし、先頭フレームと末尾フレームの差が小さい3〜6秒の動画を使うSHOULD。
...
SVG、HTML、JavaScript、実行ファイル、モデル、フォント、上記以外の音声・動画ファイルを含むパックは拒否するMUST。画像、音声、動画は拡張子だけでなくマジックバイトでも判定するMUST。
...
| `mikan.motion` | 人物ごとの無音MP4待機モーションに対応 |
