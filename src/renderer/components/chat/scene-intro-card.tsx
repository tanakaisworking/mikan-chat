...
export function SceneIntroCard({ context }: { context: ScenarioContext }) {
  return (
    <article className="overflow-hidden rounded-lg border border-border/70 bg-surface shadow-soft max-md:rounded-2xl max-md:border-white/35 max-md:bg-black/58 max-md:text-white max-md:shadow-overlay max-md:backdrop-blur-md">
...
function IntroSection({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
  return (
    <section className="grid grid-cols-[24px_minmax(0,1fr)] gap-3">
