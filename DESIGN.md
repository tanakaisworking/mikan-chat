...
新しい物語をホームから始めたときは、トークの先頭に「あなたの役 → いまの状況 → 登場人物」の導入を表示します。導入は`plot.playerProfiles`、一覧向けの短いシナリオ説明、`plot.characters`から組み立て、作者が書いた`plot.opening`はその後に続けます。チャット一覧や会話履歴から再開するときは表示しません。
