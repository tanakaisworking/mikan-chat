...
Web版はBYOK方式とし、ユーザー自身のAPIキーをブラウザタブ内だけに保持します。AIへのリクエストは設定されたOpenAI互換エンドポイントへ直接送り、mikan chatのサーバーにはキーを保存しません。Electron版も永続保存を実装するまではアプリ終了時にキーを破棄し、保存対応時はpreloadを介してOSの安全な資格情報領域を使います。
