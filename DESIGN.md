...
Web版はBYOK方式とし、ユーザー自身のAPIキーを`sessionStorage`へ保存します。同じタブの再読み込みでは復元し、タブを閉じると破棄します。AIへのリクエストは設定されたOpenAI互換エンドポイントへ直接送り、mikan chatのサーバーにはキーを保存しません。Electron版も永続保存を実装するまではアプリ終了時にキーを破棄し、保存対応時はpreloadを介してOSの安全な資格情報領域を使います。
