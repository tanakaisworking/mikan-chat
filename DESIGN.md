...
画面上の主役は、AIモデルや推論設定ではなく、ユーザーが入り込めるシナリオと会話です。ローカルLLM、API、Whisper互換音声認識、Irodori TTSは体験を支える基盤として扱い、必要になるまで技術用語を見せません。
...
Web版の運営所有APIキーはWorker Secretsだけに置き、Rendererへ渡しません。Electron版でユーザー自身のAPIキーを扱う場合は、Rendererへ直接保存せず、preloadを介してOSの安全な資格情報領域へ保存します。
...
常設サイドバーは置きません。ホームとトークの往復、必要時に開くDialog／SheetだけでMVPを構成します。
...
ホームは上部を固定App Barに切り替え、キャラクターカードを2列表示します。トーク画面以外には「ホーム / チャット」の下部タブを置き、「ホーム」はキャラクター一覧と追加、「チャット」は最近の会話を再開する「続きから」を表示します。実際のトーク画面では下部タブを表示しません。Dialogは画面内余白を8px残し、Sheetは全幅表示にします。初回設定は案内画像と接続選択を縦に積み、画面全体をスクロール可能にします。
